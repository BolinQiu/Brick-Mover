...
def from_image_state(image_path="images/image.png", model_path='runs/classify/train/weights/best.pt'):
    rec = YOLORecognizer(model_path)
    board_image = Image.open(image_path)
    return rec.recognize(board_image)

def zhuan_rec():
    rec = ZhuanRecognizer('runs/classify/train2/weights/best.pt')
...
def entry_from_local_image():
    ...
    initial_state = from_image_state(image_path='./images/image.png', model_path='./runs/classify/train2/weights/best.pt')[0]

    # zhuan_result = zhuan_rec()
    # print(zhuan_result)
    # initial_state = zhuan_result.result
    # neighbors = test_node.get_neighbors()
    neighbors = BoardState(initial_state).get_neighbors()
...
# 获取某个坐标的小方块的照片，用于微调bug，保存小范围的误差图片
def save_index_image(index):
    coords, screenshot = capture_window("砖了个砖")
    left, top, width, height = coords
    board_image = crop_image(screenshot, BOARD_AREA_POSITION)
    image_list = split_image(board_image, NUM_BOARD_ROWS, NUM_BOARD_COLS)
    num = index[0] * NUM_BOARD_COLS + index[1]
    image_list[num].save(f"images/index_{index[0]}_{index[1]}.png")


if __name__ == "__main__":
    # entry_from_local_image() //从本地的图片开始
    # exit()
